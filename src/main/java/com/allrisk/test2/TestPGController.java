...
public class TestPGController {
    ...
    @RequestMapping(value = {"/", "/testPG"}, method = RequestMethod.GET)
    public String list(Model p_Model) {
        // Call MyBatis mapper to get data from DB
        List<PSC> t_QueryResult = m_SqlSession.selectList("SELECT * FROM psc ORDER BY mesto DESC");
 
        // Add retrieved data to our model
        p_Model.addAttribute("pscData", t_QueryResult);
